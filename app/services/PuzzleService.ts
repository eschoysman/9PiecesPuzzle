...
export const getByKeyId = (keyID: number) => fetch(`json/solutions/Solution${keyID}.json`).then(response => response.json())
    .then(solutionResponse => {
        return {
            ...
            combinations: solutionResponse.combinations.map((comb: Omit<Combination,'id'>, index:number) => ({...comb,id:solutionResponse.key.code+'/'+(index+1)} as Combination)),
            template: getSolutionTemplate(solutionResponse.key)
        } as Solution
    });
